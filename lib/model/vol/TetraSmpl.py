...
# # 将Thuman2.0提供的GT smpl的pkl文件转化为可四面体化的smpl模型
...
#
#     # 指定目录
#     directory = '/home/china/lab/3D_reconstrusction/integrated_PIFu/rendering_script/THuman2.0_gt_smpl/'
#
#     # 遍历子目录
...
#
#             vs = smpl.transl_to_npy(vs, transl)
#
...
#
#             # 拼接文件路径
#             filepath = os.path.join(filepath, filename.split(".")[0]+".obj")
#
...
# # 将Thuman2.0提供的GT smpl的pkl文件转化为和3DNBF输出格式相同的json文件
# if __name__ == '__main__':
#     smpl = TetraSMPL('../data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl', '../data/tetra_smpl.npz')
#
#     import os
...
# # 将3DNBF输出的json文件转化为可四面体化的smpl模型
# if __name__ == '__main__':
#     smpl = TetraSMPL('../data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl', '../data/tetra_smpl.npz')
...
#
#     # 指定目录
#     directory_path = '/home/china/lab/3D_reconstrusction/3DNBF/data/datasets/'
#
#     for subdir in os.listdir(directory_path):
#         subdir_path = os.path.join(directory_path, subdir)
#         if os.path.isdir(subdir_path):
#             json_file = os.path.join(subdir_path, 'result_keypoints.json')
...
#
#                 for i in range(11):
#                     pose = torch.Tensor(poses[i]).float()
#                     shape = torch.Tensor(shapes[i]).float()
#                     camera = torch.Tensor(cameras[i]).float()
#
#                     pose = pose.unsqueeze(dim=0)
#                     shape = shape.unsqueeze(dim=0)
#                     camera = camera.unsqueeze(dim=0)
#
#                     vs = smpl.forward(pose, shape)
#
#                     vs = smpl.rot_vert(vs, camera)
#
#                     ts = smpl.tetrahedrons.cpu().numpy()
#
#                     filepath = os.path.join("./vis_pose", subdir)
#                     os.makedirs(filepath, exist_ok=True)
#
#                     # 从文件名中提取角度
#                     match = re.search(r'\d{3}', file_list[i])
#                     if match:
...
#
#                     # 生成文件名
#                     filename = f'smpl_{angle:03d}.obj'
#
#                     # 拼接文件路径
...
# # 将3DNBF输出的json文件转化为可四面体化的smpl模型（非批量）
# if __name__ == '__main__':
#     smpl = TetraSMPL('../data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl', '../data/tetra_smpl.npz')
#
#     import os
...
#         # cameras = np.array(data['cameras'])
#
#         pose = torch.Tensor(poses).float()
#         shape = torch.Tensor(shapes).float()
#         # camera = torch.Tensor(cameras).float()
#
#         pose = pose.unsqueeze(dim=0)
#         shape = shape.unsqueeze(dim=0)
#         # camera = camera.unsqueeze(dim=0)
