...
voxelize_cuda_cu_path = os.path.join(current_path, "cuda/voxelize_cuda.cu")
voxelize_cuda_cpp_path = os.path.join(current_path, "cuda/voxelize_cuda.cpp")
if os.path.exists(voxelize_cuda_cu_path) and os.path.exists(voxelize_cuda_cpp_path):
    voxelize_cuda = load(
        name='voxelize_cuda',
        sources=[voxelize_cuda_cpp_path, voxelize_cuda_cu_path],
        extra_ldflags=['-L/usr/local/cuda/targets/x86_64-linux/lib'],
        verbose=True)
# elif os.path.exists('./lib/model/vol/cuda/voxelize_cuda.cu') and \
#     os.path.exists('./lib/model/vol/cuda/voxelize_cuda.cpp'):
#     voxelize_cuda = load(
#         name='voxelize_cuda',
#         sources=['./lib/model/vol/cuda/voxelize_cuda.cpp',
#                  './lib/model/vol/cuda/voxelize_cuda.cu'],
#         extra_ldflags=['-L/usr/local/cuda/targets/x86_64-linux/lib'],
#         verbose=True)
else:
    raise IOError('Cannot find voxelize_cuda.cu and voxelize_cuda.cpp')
...
class Voxelization(nn.Module):
    ...
    @staticmethod
    def para_to_smpl(pose, shape):
        # transl is cat after pose
        ...
        smpl = TetraSMPL(basicModel_path, tetra_smpl_path)
        device = pose.device
        smpl = smpl.to(device=device)
        pose_list = torch.split(pose, split_size_or_sections=1, dim=0)
        shape_list = torch.split(shape, split_size_or_sections=1, dim=0)
        vs_list = []
        for pose_, shape in zip(pose_list, shape_list):
            if pose_.size() == torch.Size([1, 25, 3]):
                # Cutting transl from pose para
                pose = pose_[0, 0:24, :]
                ...
                pose = pose_[0, 0:72]
                transl = pose_[0, 72:75]
            vs = smpl.forward(pose, shape)
            vs = smpl.transl_respectively(vs, transl)
            # vs = vs.detach()
            vs = vs * 0.5
    ...
    @staticmethod
    def para_to_smpl_reverse(pose, shape):
        # transl is cat after pose
        basicModel_path = os.path.join(current_path, "smpl_data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl")
        tetra_smpl_path = os.path.join(current_path, "smpl_data/tetra_smpl.npz")
        smpl = TetraSMPL(basicModel_path, tetra_smpl_path)
    ...
    @staticmethod
    def para_to_smpl_faces(pose, shape):
        # transl is cat after pose
        basicModel_path = os.path.join(current_path, "smpl_data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl")
        tetra_smpl_path = os.path.join(current_path, "smpl_data/tetra_smpl.npz")
        smpl = TetraSMPL(basicModel_path, tetra_smpl_path)
    ...
    @staticmethod
    def para_to_smpl_faces_reverse(pose, shape):
        # transl is cat after pose
        basicModel_path = os.path.join(current_path, "smpl_data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl")
        tetra_smpl_path = os.path.join(current_path, "smpl_data/tetra_smpl.npz")
        smpl = TetraSMPL(basicModel_path, tetra_smpl_path)
