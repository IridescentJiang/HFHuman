...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--use_mask_for_rendering_low_res',default=True) # Not used in the paper.
        ...
        parser.add_argument('--use_SMPL_depth_map', default=False)
        parser.add_argument('--use_depth_map', default=True)  # Used in the paper, but may not lead to better results if accurate depth map prediction cannot be achieved.
        parser.add_argument('--depth_in_front', default=True) # Used in the paper. Only cat depth map with image as a whole, not cat depth map feature with image feature at each point.
        ...
        parser.add_argument('--use_reference_depth_map_for_depth_training', default=True)
        parser.add_argument('--use_smpl_depth_map_for_depth_training', default=False)
        parser.add_argument('--second_stage_depth', default=True) # Set false when only use the first step depth
        ...
        parser.add_argument('--use_smpl', default=False)
        parser.add_argument('--use_smpl_gt', default=False)

        parser.add_argument('--use_smpl_para', default=True)
        parser.add_argument('--use_smpl_para_opt', default=False)
        parser.add_argument('--use_smpl_para_gt', default=False)

        parser.add_argument('--use_smpl_x', default=False)

        parser.add_argument('--use_transformer_encoder', default=False)

        smpl_para_learning = True
        smpl_para_learning_pretrained_epoch = 0
        smpl_para_learning_after_trained_epoch = 0
        if smpl_para_learning:
            smpl_para_learning_pretrained_epoch = 0
            smpl_para_learning_after_trained_epoch = 10

        parser.add_argument('--smpl_para_depth_guidance_learning', default=smpl_para_learning)
        parser.add_argument('--smpl_para_learning_pretrained_epoch', type=int, default=smpl_para_learning_pretrained_epoch)
        parser.add_argument('--smpl_para_learning_after_trained_epoch', type=int, default=smpl_para_learning_after_trained_epoch)

        parser.add_argument('--film_fusion', default=True)

        parser.add_argument('--smpl_test_opt', default=True)  # Open only when testing
        
        parser.add_argument('--num_epoch', type=int, default=50 + smpl_para_learning_pretrained_epoch, help='num epoch to train') # depends on user's preference as well. (e.g. how large is your train set / Did validation error stop decreasing etc.)
        ...
        parser.add_argument('--learning_rate_G', type=float, default=5e-4, help='adam learning rate for low res')
        ...
        parser.add_argument('--use_sampled_points', default=True)
        parser.add_argument('--use_sampled_points_smpl', default=False)

        parser.add_argument('--debug_mode', default=False)

        parser.add_argument('--no_images', default=True)
        ...
        parser.add_argument('--hg_dim_high_res', type=int, default=256)
        parser.add_argument('--ve_3D_dim', type=int, default=32)
        ...
        parser.add_argument('--mlp_dim_low_res_smpl', nargs='+', default=[288, 1024, 512, 256, 128, 1], type=int,
                             help='# of dimensions of mlp when using smpl. no need to put the first channel')

        parser.add_argument('--mlp_dim_low_res_transformer', nargs='+', default=[32, 1024, 512, 256, 128, 1], type=int,
                             help='# of dimensions of mlp. no need to put the first channel')
        ...
        parser.add_argument('--loadSizeDepth', type=int, default=224, help='load size of input depth map')
        parser.add_argument('--loadSizeDepthOpt', type=int, default=512, help='load size of input depth map')
