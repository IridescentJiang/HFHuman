...
def train(opt):
    ...
    if load_model:  # by default is false
        ...
        modeldepthfilter_path = "checkpoints/Date_27_Feb_24_Time_14_34_15/depthfilter_model_state_dict.pickle"  # Date_08_Jan_22_Time_02_03_43 is folder to load
    ...
    for epoch in range(start_epoch, num_of_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if opt.second_stage_depth:
                ...
                render_tensor = train_data['original_high_res_render'].to(
                    device=device)  # the renders. Shape of [Batch_size, Channels, Height, Width]
                coarse_depth_map_tensor = train_data['coarse_depth_map'].to(
                    device=device)  # shape of [batch, 1,1024,1024]
                smpl_depthmap = train_data['smpl_depth_map'].to(
                    device=device)
                reference_depthmap = train_data['reference_depth_map'].to(
                    device=device)
                ...
                render_tensor = train_data['original_high_res_render'].to(
                    device=device)  # the renders. Shape of [Batch_size, Channels, Height, Width]
                # render_tensor = train_data['render_low_pifu'].to(
                #     device=device)  # the renders. Shape of [Batch_size, Channels, Height, Width]
                smpl_depthmap = train_data['smpl_depth_map'].to(
                    device=device)
                reference_depthmap = train_data['reference_depth_map'].to(
                    device=device)
                depth_map_tensor = train_data['depth_map'].to(device=device)  # shape of [batch, 1,1024,1024]
                center_indicator_tensor = train_data['center_indicator'].to(
                    device=device)  # shape of [batch, 1,1024,1024]
            ...
            error = depthfilter.forward(images=render_tensor, smpl_depthmap=smpl_depthmap,
                                        reference_depthmap=reference_depthmap,groundtruth_depthmap=depth_map_tensor)
        ...
        with torch.no_grad():
            if True:
                ...
                # save models
                with open('%s/%s/depthfilter_model_state_dict.pickle' % (opt.checkpoints_path, opt.name),
                          'wb') as handle:
                ...
                for gen_idx in tqdm(range(1)):
                    ...
                    if opt.second_stage_depth:
                        ...
                        image_tensor = train_data['original_high_res_render'].to(
                            device=device)  # the renders. Shape of [Batch_size, Channels, Height, Width]
                        coarse_depth_map_tensor = train_data['coarse_depth_map'].to(
                            device=device)  # shape of [batch, 1,1024,1024]
                        ...
                        # smpl_depthmap = train_data['smpl_depth_map'].to(
                        #     device=device)
                        # smpl_depthmap = torch.unsqueeze(smpl_depthmap, 0)
                        reference_depthmap = train_data['reference_depth_map'].to(
                            device=device)
                        reference_depthmap = torch.unsqueeze(reference_depthmap, 0)
                    ...
                    depthfilter.filter(image_tensor, reference_depthmap)  # forward-pass
                    ...
                    if opt.second_stage_depth:
                        save_differencemap_path = save_path[:-4] + '_generated_differencemap.png'
                        save_differencemap = generated_depth_map - coarse_depth_map_tensor.detach().cpu().numpy()[0,
                                                                   0:1, :, :]
                    ...
                    save_depthmap_path = save_path[:-4] + '_generated_depthmap.png'

                    save_img = (np.transpose(image_tensor[0, :3, ...].detach().cpu().numpy(),
                                             (1, 2, 0)) * 0.5 + 0.5) * 255.0
