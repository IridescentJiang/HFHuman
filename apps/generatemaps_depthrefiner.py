...
from lib.data.DepthDataset import DepthDataset, DepthInferenceDataset
...
# if False, generate Coarse depth maps. If True, generate refined depth maps (Second-stage maps)
# remember to change the options' --second_stage_depth synchronize with this flags
generate_refined_trained_depth_maps = True


if generate_for_inference:
    if generate_refined_trained_depth_maps:
        trained_depth_maps_path = "rendering_script/inference/trained_depth_maps_refer"
    else:
        trained_depth_maps_path = "rendering_script/inference/trained_coarse_depth_maps_refer"
    batch_size = 1
else:
    if generate_refined_trained_depth_maps:
        trained_depth_maps_path = "trained_depth_maps_refer"
    else:
        trained_depth_maps_path = "trained_coarse_depth_maps_refer"
...
def generate_maps(opt):
    ...
    if generate_for_inference:
        infer_dataset = DepthInferenceDataset(opt)
        infer_dataset.is_train = False
        infer_data_loader = DataLoader(infer_dataset,
                                       batch_size=batch_size, shuffle=False,
                                       num_workers=opt.num_threads, pin_memory=opt.pin_memory)
        print('infer loader size: ', len(infer_data_loader))
        data_loader_list = [('first_data_loader', infer_data_loader)]
    else:
        if generate_for_buff_dataset:
    ...
    # load model
    if generate_refined_trained_depth_maps:
        modeldepthfilter_path = "pretrained_model/depthreferfilter_model_state_dict_2_stage_v2.pickle" # Date_08_Jan_22_Time_02_03_43 is the folder to use.
    else:
        modeldepthfilter_path = "pretrained_model/depthreferfilter_model_state_dict_1_stage_v1.pickle" # Date_06_Jan_22_Time_02_37_32 is the folder to use.
        ...
        for description, data_loader in data_loader_list:
            ...
            for idx, batch_data in enumerate(data_loader):
                ...
                if generate_refined_trained_depth_maps:
                    ...
                    reference_depthmap = batch_data['reference_depth_map'].to(
                        device=device)
                ...
                depthfilter.filter( render_tensor, reference_depthmap ) # forward-pass
